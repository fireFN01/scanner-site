...
export default function App() {
  ...
  const handleAudit = async () => {
    ...
    setResult(null);
    try {
      // 1. DNS Lookup via Google DNS API - Pubblico
      const dnsRes = await axios.get(`https://dns.google/resolve?name=${domain}&type=A`);
      const ips = dnsRes.data.Answer?.map((a: any) => a.data) || ["N/A"];

      // 2. SSL Labs API - Pubblico, solo grade
      const sslRes = await axios.get(`https://api.ssllabs.com/api/v3/analyze?host=${domain}&publish=off&startNew=off`);
      ...
      // 3. Security Headers - via nostro proxy per evitare CORS
